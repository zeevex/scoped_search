...
While <b>search_for</b> is the default named search scope, you can define multiple
scopes by naming them in other ways:
...
Note that because all of the above provided names for the search scopes, <b>search_for</b>
has not been defined.  This is useful if some other plugin or your own code has already
defined a <b>search_for</b> method.

If you'd rather select the columns to search at runtime, you can do that with the 
default "scoped_search" scope created when you invoke "acts_as_scoped_search"
or "searchable_on" in any way.  It's the most general method provided.
...
Finally, you can specify additional columns to search in a named scoped_search when
you invoke it.  The additional columns will only be used for that one query.  For
example:
